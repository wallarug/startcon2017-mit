...
class JoyTeleop:
	...
	#rospy.Subscriber('/vesc/low_level/ackermann_cmd_mux/input/teleop', AckermannDriveStamped, self.ack_callback)
	
        # Run a low-freq action updater
        rospy.Timer(rospy.Duration(2.0), self.update_actions)

	#rospy.logerr("publishers: {0} \nal_clients: {1} \nsrv_clients: {2} \nservice_types: {3} \nmessage_types: {4} \ncommand_lists: {5} \noffline_actions: {6} \noffline_services: {7} \n".format(self.publishers, 
	#						self.al_clients, 
	#						self.srv_clients, 
	#						self.service_types,
	#						self.message_types,
	...
	rospy.logerr("data: {0}".format(data))
	

    def ai_team_is_trash(self, data):
	pass
        ...
        try:
            for c in self.command_list:
                if self.match_command(c, data.buttons):
                    ...
                    #rospy.logerr("c: {0}  | data: {1}\n".format(c, data.buttons))
    ...
    def run_topic(self, c, joy_state):
        ...
        #rospy.logerr("cmd: {0} | c: {1} | msg: {2}".format(cmd, c, msg))
        if 'message_value' in cmd:
            for param in cmd['message_value']:
		#if param['target'] == 'drive.speed' and c == 'spam_bot':
		#    self.set_member(msg, param['target'], 1.0)
		#elif param['target'] == 'drive.steering_angle' and c == 'spam_bot':
		#    self.set_member(msg, param['target'], 0.3)
		#else:
        ...
        #rospy.logerr("cmdtopic: {0} | \nmsg: {1}".format(cmd['topic_name'], msg))
